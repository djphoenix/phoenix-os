...
short __inline inports(short port){
	short c; asm("inw %w1, %w0":"=a"(c):"d"(port)); return c;
}
uint __inline inportl(short port){
	uint c; asm("inl %w1, %d0":"=a"(c):"d"(port)); return c;
}
void __inline outportb(short port, char c){
	asm("outb %b0, %w1"::"a"(c),"d"(port));
}
void __inline outports(short port, short c){
	asm("outw %w0, %w1"::"a"(c),"d"(port));
}
void __inline outportl(short port, uint c){
