...
size_t readelf(Process *process, Stream *stream) {
	if (
		(stream->read(&Elf, sizeof(Elf)) != sizeof(Elf)) ||
	...
	ELF64SECT *sections = (ELF64SECT*)Memory::alloc(sizeof(ELF64SECT)*Elf.shnum);
	...
	for (uint32_t i = 0; i < Elf.shnum; i++) {
		...
		for (size_t r = 0; r < relcnt; r++) {
			...
			char *symname; uintptr_t addr, offset;
			if (sym.info == 3) {
